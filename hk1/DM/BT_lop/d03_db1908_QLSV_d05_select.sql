...
go
--xem kq thi bao gom luon ten sv
select a.*,b.TenSV
from tbDiemKQ a join tbSinhVien b
on a.MaSV=b.MaSV
go
select *
from tbDiemKQ a join tbSinhVien b
...
create view vwSinhVienThiC
as
select a.*,b.Diem
from tbSinhVien a join tbDiemKQ b on a.MaSV=b.MaSV 
where MaMH ='C';
go
--xem ds sv da du thi mon C (thong qua view vwSinhVienThiC)
...
--them 1 sv nam vo bang tbSinhVien--thuc te insert zo bang
insert vwSinhVienNam values('s20','20001230','nghia@gamil.com',1,'1910-M1','Huu Nghia')
go
select * from vwSinhVienNam
go
select * from tbSinhVien
go
--them 1 sv nu vo bang tbSinhVien thong qua view vwSinhVienNam:Vo Ly chua?-- no cap nhat zp thg bang
insert vwSinhVienNam values('s21','20001230','gian@gamil.com',1,'1910-M1','Nguyen Thi Giang')
go
-- cap nhat nhat lai gioi tinh cua ban ma so s21
update vwSinhVienNam set GioiTinh=0 where MaSV='s21'
go
-- sua lai dinh nghia view nam sinh them dk with check option:ko cho phep them 1 nu sinh zo DB thong qua view nam sinh
...
('s22','20011020','chi2gmail.com',0,'1910-M1','Dang Tieu Chi')
go