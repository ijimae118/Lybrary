...
select a.MaSV,b.TenSV, count(*) [so luot thi] 
	from  tbDiemKQ a , tbSinhVien b 
	where a.MaSV=b.MaSV
	group by a.MaSV,b.TenSV
	having count(*) >=2
	order by 3 desc
go
-- lay them ten sv
...
-- => ds ket qua thi mon C va mon HTML(lay het)
...
select * from tbDiemKQ
where MaMH ='C' or MaMH='HTML' order by MaMH
go

-- demo INTERSECT(lay giao)
...
select * from
(select MaSV, MaMH, Diem from tbDiemKQ) abc
pivot
(
